...
# Database settings
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///rag/shoe_metadata.db")
...
# API settings
_cors_origins_str = os.getenv("CORS_ORIGINS", "http://localhost:3000,http://localhost:5000")
_cors_origins = [origin.strip() for origin in _cors_origins_str.split(",")]
...
LOG_DIR = BASE_DIR / "rag" / "logs"
LOG_DIR.mkdir(parents=True, exist_ok=True)

LOGGING_CONFIG = {
    "level": "DEBUG" if DEBUG else "INFO",
    "format": "%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    "file": str(LOG_DIR / "rag_system.log")
}
...
# Warn if using default secret in non-debug mode
if JWT_CONFIG['secret_key'] == 'INSECURE-DEFAULT-CHANGE-ME' and not DEBUG:
    import warnings
    warnings.warn(
